...
const PptxStudio = {
  ...
  _drawTextRuns(ctx, el, px, py, pw, ph, scaleY, isPreview) {
    ...
    lines.forEach(line => {
      ...
      const font = `${run.italic ? 'italic ' : ''}${run.bold ? 'bold ' : ''}${fsPx.toFixed(1)}px ${this._safeFont(run.fontFamily)}`;
      ctx.font    = font;
      ...
      if (ctx.measureText(line).width > maxW && maxW > 0) {
        if (this._overflowMode === 'truncate') {
          ...
          ctx.font = `${run.italic ? 'italic ' : ''}${run.bold ? 'bold ' : ''}${newFs.toFixed(1)}px ${this._safeFont(run.fontFamily)}`;
        }
      }
   ...
   * para preservar a aparência condensed/bold do PPTX.
   */
  _safeFont(family) {
    if (!family || family === '+mj-lt' || family === '+mn-lt') {
      return '"Barlow", "Helvetica Neue", Arial, sans-serif';
    }
    // Condensed variants (57 Cn, 67 MdCn, 77 BdCn, 87 HvCn, 93 BlkEx, etc.)
    ...
    // Semi-condensed / Extended (53 Ex, 63 MdEx, 73 BdEx)
    ...
    // Any other HelveticaNeueLT or Helvetica
