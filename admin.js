...
const Admin = {
            ...
            <tr>
              ...
              <td>
                <strong style="color:var(--green);font-size:15px">
                  ${parseFloat(String(r.productivity || 0).replace(',','.')).toLocaleString('pt-BR', {minimumFractionDigits:1, maximumFractionDigits:1})}
                </strong>
  ...
  async editRecord(id) {
    // Admin delega para User.editRecord que abre o formulário de edição.
    // AccessControl.can('can_edit_card') já retorna true para admin,
    // portanto a checagem interna do User.editRecord não bloqueia.
    try {
      if (typeof User !== 'undefined' && User.editRecord) {
        await User.editRecord(id);
      }
    } catch (err) {
      console.error('[Admin.editRecord]', err);
      App.Toast.show('Erro ao abrir edição: ' + (err.message || 'tente novamente.'), 'error');
    }
  },
