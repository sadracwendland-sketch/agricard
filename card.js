...
const PptxParser = {
  ...
  async parseFile(fileOrBuffer) {
    ...
    const elements = this._parseLayoutPlaceholders(layoutXml, slideW, slideH, themeColors);
  ...
  _parseLayoutPlaceholders(layoutXml, slideW, slideH, themeColors = {}) {
    const parser = new DOMParser();
    ...
    const sps = Array.from(doc.getElementsByTagNameNS('*', 'sp'));
    for (const sp of sps) {
      const el = this._parseLayoutShape(sp, slideW, slideH, themeColors);
  ...
  _parseLayoutShape(sp, slideW, slideH, themeColors = {}) {
...
const CardRenderer = {
  ...
  async _drawElement(ctx, el, data, slideW, slideH, canvasW, canvasH) {
    ...
    // Posição Y: centraliza verticalmente no box
    const textH  = fsPx * 1.2;
    const drawY  = by + Math.max(tPad, (bh - textH) / 2);
  ...
  /**
   * Mapeamento de fontes HelveticaNeueLT Pro → substitutos web disponíveis
   *
   * Variantes do PPTX → Google Fonts equivalentes:
   *   HelveticaNeueLT Pro 45 Lt       → Barlow Light (300)
   *   HelveticaNeueLT Pro 55 Roman    → Barlow Regular (400)
   *   HelveticaNeueLT Pro 65 Md       → Barlow Medium (500)
   *   HelveticaNeueLT Pro 75 Bd       → Barlow Bold (700)
   *   HelveticaNeueLT Pro 85 Hv       → Barlow ExtraBold (800)
   *   HelveticaNeueLT Pro 95 Blk      → Barlow Black (900)
   *   HelveticaNeueLT Pro 47 LtCn     → Barlow Condensed Light (300)
   *   HelveticaNeueLT Pro 57 Cn       → Barlow Condensed Regular (400)
   *   HelveticaNeueLT Pro 67 MdCn     → Barlow Condensed Medium (500)
   *   HelveticaNeueLT Pro 77 BdCn     → Barlow Condensed Bold (700)
   *   HelveticaNeueLT Pro 87 HvCn     → Barlow Condensed ExtraBold (800)
   *   HelveticaNeueLT Pro 93 BlkEx    → Barlow Condensed Black (900)
   *   HelveticaNeueLT Pro 53 Ex       → Barlow Semi Condensed (400)
   *   HelveticaNeueLT Pro 63 MdEx     → Barlow Semi Condensed Medium (500)
   *   HelveticaNeueLT Pro 73 BdEx     → Barlow Semi Condensed Bold (700)
   */
  _HELVETICA_MAP: {
    // Condensed variants (Cn / Ex variants)
  ...
  /** Fonte segura: mapeia HelveticaNeueLT Pro → Barlow (Google Fonts) */
  _safeFont(family) {
    if (!family) return '"Barlow", Arial, sans-serif';

    // Busca mapeamento exato primeiro
    if (this._HELVETICA_MAP[family]) return this._HELVETICA_MAP[family];

    // Variantes condensed genéricas
  ...
  /**
   * Garante que as fontes Barlow estejam carregadas no browser
   * antes de renderizar o canvas.
   */
  async _ensureFontsLoaded() {
    if (!document.fonts) return;
    const fontsToLoad = [
      // Barlow Condensed – substituto principal das variantes condensed Helvetica
      { weight: '400', family: '"Barlow Condensed"' },
      { weight: '700', family: '"Barlow Condensed"' },
      { weight: '800', family: '"Barlow Condensed"' },
      { weight: '900', family: '"Barlow Condensed"' },
      { weight: '300', family: '"Barlow Condensed"' },
      // Barlow – substituto das variantes regulares
      { weight: '400', family: '"Barlow"' },
      { weight: '700', family: '"Barlow"' },
      { weight: '800', family: '"Barlow"' },
      { weight: '900', family: '"Barlow"' },
      // Barlow Semi Condensed
      { weight: '400', family: '"Barlow Semi Condensed"' },
      { weight: '700', family: '"Barlow Semi Condensed"' },
    ];
    try {
      await Promise.all(
        fontsToLoad.map(f =>
          document.fonts.load(`${f.weight} 16px ${f.family}`).catch(() => {})
        )
      );
    } catch {}
  },
