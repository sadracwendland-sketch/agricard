...
const PptxParser = {
  ...
  async parseFile(fileOrBuffer) {
    ...
    // Deduplica pelo fname (evita desenhar a mesma imagem duas vezes)
    const seen = new Set();
    const allImages = [];
    ...
    // ── 7. Placeholders de texto ──────────────────────────
    const elements = this._parseLayoutPlaceholders(layoutXml, slideW, slideH);
  ...
  _parseLayoutPlaceholders(layoutXml, slideW, slideH) {
    const parser = new DOMParser();
    ...
    const sps = Array.from(doc.getElementsByTagNameNS('*', 'sp'));
    for (const sp of sps) {
      const el = this._parseLayoutShape(sp, slideW, slideH);
  ...
  _parseLayoutShape(sp, slideW, slideH) {
    ...
    for (const lvl of ['lvl5pPr','lvl4pPr','lvl3pPr','lvl2pPr','lvl1pPr']) {
      ...
      if (pPr) {
        if (!defRpr) {
          ...
          defRpr = rprEls.length ? rprEls[0] : null;
        }
        const algnAttr = pPr.getAttribute('algn');
        if (algnAttr && align === 'left') {
          align = { l:'left', ctr:'center', r:'right', just:'left' }[algnAttr] || 'left';
        }
      }
    }

    // Tamanho de fonte
    const szRaw      = defRpr?.getAttribute('sz');
    ...
    // Família de fonte
    const latin      = defRpr ? this._nsFind(defRpr, 'latin') : null;
    const fontFamily = latin?.getAttribute('typeface') || 'Arial';

    // Cor do texto
    const SCHEME_COLORS = {
      'tx1':'#000000', 'dk1':'#000000',
      'bg1':'#FFFFFF', 'lt1':'#FFFFFF',
      'tx2':'#1F3864', 'dk2':'#0E2841',
      'bg2':'#E8E8E8', 'lt2':'#E8E8E8',
    ...
    let color = null;
    if (defRpr) {
      const solidFills = defRpr.getElementsByTagNameNS('*', 'solidFill');
      if (solidFills.length) {
        ...
        if (srgb) {
          ...
          if (scheme) color = SCHEME_COLORS[scheme.getAttribute('val')] || null;
        }
      }
    }
    ...
    if (placeholders.length > 1) {
      return {
        ...
        separator: '/',
        x, y, w, h,
        fontSizePt, bold, italic: false, fontFamily, color, align,
...
const CardRenderer = {
  ...
  /** Fonte segura: fallback para Arial se a fonte não estiver disponível */
  _safeFont(family) {
    if (!family) return 'Arial, sans-serif';
    // HelveticaNeueLT Pro não está disponível nos browsers → usa Helvetica/Arial
    if (family.includes('HelveticaNeueLT')) return '"Helvetica Neue", Helvetica, Arial, sans-serif';
    if (family.includes('Helvetica'))      return '"Helvetica Neue", Helvetica, Arial, sans-serif';
